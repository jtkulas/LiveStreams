...
[The `echo: false` option disables the printing of code (only output is displayed).]{style="color:#bada55"}

```
1. fenced divs not as responsive as bracketed spans (line numbers 14 & 29 below). Note we're not operating on the most recent version of Quarto (currently using Version `r quarto::quarto_version()`)
